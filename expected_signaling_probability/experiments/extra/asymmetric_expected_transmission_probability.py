...
def plot_asymmetric_expected_transmission_probability(
    ...
    plot_title(f"Asymmetric Expected Transmission Probability ($N =$ {LatexStrings.n_samples_to_sci(all_stats_A_to_B[0].n)})")
    plt.legend()
    plt.tight_layout()

    if save:
        save_plot("asymmetric_expected_transmission_probability")
    plt.show()
