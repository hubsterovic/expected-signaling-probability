...
def compute_asymmetric_expected_correlation_probability(n_samples: int, d_A_min: int, d_A_max: int, d_B: int, direction: Direction) -> list[Stats]:
    assert d_A_min <= d_A_max
    dims_A = [d for d in range(d_A_min, d_A_max + 1)]
    all_stats = []
    for d_A in dims_A:
        samples = expected_correlation_probability(n_samples, d_A, d_B, direction)
...
def add_plot_power_law_fit(x: np.typing.NDArray, y: np.typing.NDArray, direction: Direction):
    ...
    plt.plot(
        x_fit,
        y_fit,
        label=rf"Fit: {LatexStrings.EXPECTED_CORRELATION_PROBABILITY_A_TO_B if direction == Direction.A_TO_B else LatexStrings.EXPECTED_CORRELATION_PROBABILITY_B_TO_A} $ \propto d_A^{{{slope:.2f} \pm {stderr:.2f}}}$",
        linestyle="--",
...
def add_plot_scatter(x: np.typing.NDArray, y: np.typing.NDArray, direction: Direction):
    plt.scatter(
        ...
        color="blue" if direction == Direction.A_TO_B else "red",
        s=50,
        label=rf"Data: {LatexStrings.EXPECTED_CORRELATION_PROBABILITY_A_TO_B if direction == Direction.A_TO_B else LatexStrings.EXPECTED_CORRELATION_PROBABILITY_B_TO_A}",
    )


def plot_asymmetric_expected_correlation_probability(
    ...
    plt.title("Asymmetric Expected Correlation Probability" + " " + f"(N = {LatexStrings.n_samples_to_sci(all_stats_A_to_B[0].n)})")
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    d_A_max = 20
    d_B = 2
    all_stats_A_to_B = compute_asymmetric_expected_correlation_probability(n_samples, d_A_min, d_A_max, d_B, Direction.A_TO_B)
    all_stats_B_to_A = compute_asymmetric_expected_correlation_probability(n_samples, d_A_min, d_A_max, d_B, Direction.B_TO_A)

    plot_asymmetric_expected_correlation_probability(all_stats_A_to_B, all_stats_B_to_A)
